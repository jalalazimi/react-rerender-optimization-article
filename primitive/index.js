        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-8 gap-3 my-6 mx-2">
          {users.map((user, id) => (
            <div className="p-1" key={id}>
              <div
                className="bg-white px-6 py-8 rounded-md shadow-lg text-center hover:shadow-xl transition duration-150 ease-in-out">
                <div className="mb-3">
                  <img
                  ...
                  onClick={() => {
                    like(prev => {
                      if (likedUsers.has(user.id)) {
                        prev.delete(user.id)
                        return new Map(prev)
                      } else {return new Map(prev.set(user.id, user.login))}
                    })
                  }}
                  role="button"
